...
class TransferSagaHappyPathIT {
    ...
    @Test
    void transfer_allFeignCallsSucceed_createsFourOutboxEventsAndBothLegsCompleted() {
        ...
        stubFor(post(urlPathMatching("/internal/accounts/.*/balances/.*/commit"))
                .willReturn(aResponse().withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody(accountJson(sourceAccountId, "900.00"))));
        ...
        var response = restTemplate.exchange("/api/transactions/transfer",
        ...
        assertThat(txns).allMatch(t -> t.getStatus() == TransactionStatus.COMPLETED);

        // 4 outbox events: created+categorized for each leg
        assertThat(outboxEventRepository.count()).isGreaterThanOrEqualTo(4);
